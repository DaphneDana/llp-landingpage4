      ...
      <header className="h-20 bg-white shadow-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {[
              { jp: "ホーム", en: "HOME" },
              { jp: "サービス", en: "SERVICE", active: true },
              { jp: "事業内容", en: "BUSINESS" },
              { jp: "実績", en: "WORKS" },
              { jp: "会社概要", en: "COMPANY" },
              { jp: "採用情報", en: "RECRUIT" },
            ].map((item, index) => (
              <div key={index} className={`text-center cursor-pointer hover:opacity-80 transition-opacity ${item.active ? 'opacity-100' : ''}`}>
                <div
                  className={`text-sm ${item.active ? 'text-[#1E3A8A]' : 'text-[#6B7280]'}`}
                  style={{ fontFamily: "'Hiragino Kaku Gothic ProN', 'Hiragino Sans', 'Yu Gothic', 'Meiryo', serif" }}
                >
                  {item.jp}
                </div>
                <div
                  className={`text-sm font-bold ${item.active ? 'text-[#F59E0B]' : 'text-[#1E3A8A]'}`}
